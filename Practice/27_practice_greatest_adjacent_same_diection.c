...
int main()
{
    ...
    unsigned num = 0;
    unsigned i, j, data1, data2;

    for (i = 0; i < n - 3; i++)
    {
        for (j = 0; j < n - 3; j++)
        {
            unsigned high = gird[i][j] * gird[i][j + 1] * gird[i][j + 2] * gird[i][j + 3];
            unsigned value = gird[j][i] * gird[j + 1][i] * gird[j + 2][i] * gird[j + 3][i];
            data1 = gird[i][j] * gird[i + 1][j + 1] * gird[i + 2][j + 2] * gird[i + 3][j + 3];
            data2 = gird[i][n - j - 1] * gird[i + 1][n - j - 2] * gird[i + 2][n - j - 3] * gird[i + 3][n - j - 4];
            num = max(num, max(high, max(value, max(data1, data2))));
        }
    }
    printf("\nThe product of these diagonal number is 26 * 63 * 78 * 14:\n %d\n\n", num);
    return 0;
}
